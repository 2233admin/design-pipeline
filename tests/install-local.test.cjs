...
function fixture() {
  ...
  fs.cpSync(skillSource, source, { recursive: true });
...
test("failed replacement preserves the previous installed tree", () => {
  ...
  fs.rmSync(path.join(item.source, "scripts", "designer-pipeline.cjs"));
...
test("a source with SKILL.md but missing a required runtime file is rejected", () => {
  const item = fixture();
  fs.rmSync(path.join(item.source, "scripts", "designer-pipeline.cjs"));
  ...
  assert.match(result.stderr, /designer-pipeline\.cjs/);
...
test("a packaged installer defaults its source to the extracted package root", () => {
  ...
  const skills = path.join(root, "skills");
  const target = path.join(skills, "design-pipeline");
  fs.cpSync(skillSource, packageRoot, { recursive: true });
